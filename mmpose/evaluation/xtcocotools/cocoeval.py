...
class COCOeval:
    ...
    def computeOks(self, imgId, catId):
        ...
        for j, gt in enumerate(gts):
            ...
            limbclass_g = self.limbdecision(vg)
            ...
            for i, dt in enumerate(dts):
                if p.iouType == 'keypoints_wholebody':
                    ...
                    d = np.array(dt['keypoints'])
                    limbclass_d = np.array(dt['limb_class'])
                ...
                if self.use_area:
                    ...
                    exp = np.exp(-e)
                for focused_keypoint in focus_keypoints:
                    exp[focused_keypoint] = 0

                if k1 > 0:
                    e=e[vg > 0]
                    exp = exp[vg > 0]
                ious[i, j] = np.sum(exp) / exp.shape[0]
        return ious
